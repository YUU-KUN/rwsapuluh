<template>
    <div class="px-6 laptop:px-40">
        ...
        <div class="grid grid-cols-1 laptop:grid-cols-3 gap-4 w-full mb-6 laptop:mb-40">
            <div @click="detailAchievement(id)" v-for="{ id, title, image_url, description, categories } in achievements"
                :key="id" class="rounded-2xl bg-white p-6 text-left laptop:px-6 laptop:py-9 hover:cursor-pointer">
                <div class="relative mb-2">
                    <img :src="image_url" onerror="this.onerror=null;this.src='/assets/img/activity.png'" alt=""
                ...
                <p class="uppercase text-primary text-10 mb-1 laptop:hidden">{{ categories.length > 0 ?
                    categories[0].category.name : '' }}</p>
        ...
        <div class="grid grid-cols-1 laptop:grid-cols-3 gap-4 w-full mb-6 laptop:mb-40">
            <div @click="detailActivity(id)"
                v-for="{ id, created_at, image_url, title, description, categories } in top_activities" :key="id"
                class="rounded-2xl bg-white p-6 text-left laptop:px-6 laptop:py-9 hover:cursor-pointer">
                <div class="relative mb-2">
                    <img :src="image_url" onerror="this.onerror=null;this.src='/assets/img/activity.png';" alt=""
                        class="rounded-2xl mb-2 h-[194px] w-full laptop:mb-5 laptop:h-[297px] object-cover">
                    <div class="absolute bg-white rounded-lg p-1 top-2 left-2 w-7 h-7 text-center laptop:w-14 laptop:h-14">
