<template>
    <div class="px-6 laptop:px-40">
        ...
        <p class="font-bold text-16 mb-9 text-center laptop:text-40">Kegiatan di RW 10</p>
        ...
        <div class="grid grid-cols-1 laptop:grid-cols-3 gap-4 w-full mb-6 laptop:mb-40">
            <div @click="detailActivity(id)"
                v-for="{ id, title, image_url, description, categories, created_at } in activities" :key="id"
...
export default {
    ...
    methods: {
        getActivities() {
            this.axios.get(`activity`).then((response) => {
